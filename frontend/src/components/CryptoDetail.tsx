...
export function CryptoDetail({ crypto, priceHistory, priceChange7d, priceChange1y }: CryptoDetailProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-6 shadow-sm">
      <div className="flex items-center gap-4">
        {crypto.image && (
          <div className="relative w-16 h-16">
            <Image src={crypto.image} alt={crypto.name} fill className="object-contain" />
          </div>
        )}
        <div>
      ...
      <div>
        ...
        <p className="text-4xl font-black text-gray-900">{formatPrice(crypto.current_price)}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={item.label} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
            ...
            <div className={`flex items-center gap-1 font-bold ${item.val >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              {item.val >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex justify-between items-center">
        <div>
          ...
          <p className="font-bold text-gray-900">{formatMarketCap(crypto.market_cap)}</p>
        </div>
      ...
      <div className="pt-4">
        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-indigo-500" /> 価格推移 (1年)
        </h3>
            ...
            <LineChart data={priceHistory}>
              ...
              <Tooltip 
                ...
                formatter={(val: unknown) => {
                    ...
                    const priceText = isNaN(numericVal) ? String(val) : formatPrice(numericVal);
